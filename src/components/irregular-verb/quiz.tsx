...
export function Quiz() {
    ...
    const checkAnswer = () => {
        const isPastCorrect = answers.past.toLowerCase().trim() === currentVerb.past.split("/")[0].toLowerCase().trim()
        const isParticipleCorrect =
            answers.participle.toLowerCase().trim() === currentVerb.participle.split("/")[0].toLowerCase().trim()
    ...
    if (quizFinished) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center bg-card border border-border rounded-2xl animate-in fade-in zoom-in duration-500">
                ...
                <p className="text-muted-foreground mb-8">Bạn đã hoàn thành bài kiểm tra với số điểm:</p>
                <div className="text-6xl font-black text-primary mb-10">
                    {score} / {quizVerbs.length}
                </div>
                <Button onClick={restartQuiz} className="gap-2 h-12 px-8 rounded-xl text-lg font-semibold">
        ...
        <div className="max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-muted-foreground bg-secondary px-3 py-1 rounded-full">
                    Câu hỏi {currentIndex + 1} / {quizVerbs.length}
                </span>
            </div>

            <div className="p-8 bg-card border border-border rounded-2xl shadow-sm">
                <div className="text-center mb-10">
                    <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Động từ nguyên thể</span>
                    <h3 className="text-5xl font-black text-foreground mt-2">{currentVerb.base}</h3>
                    <p className="text-muted-foreground mt-4 italic">Nghĩa: {currentVerb.meaning}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-sm font-semibold ml-1">Thì quá khứ (V2)</label>
                        <div className="relative">
                            <Input
                                ...
                                className={cn(
                                    ...
                                    submitted &&
                                    (answers.past.toLowerCase().trim() === currentVerb.past.split("/")[0].toLowerCase().trim()
                                        ? "border-green-500 bg-green-500/5"
                                        : "border-destructive bg-destructive/5"),
                                )}
                                value={answers.past}
                                onChange={(e) => setAnswers({ ...answers, past: e.target.value })}
                                disabled={submitted}
                            />
                            {submitted &&
                                (answers.past.toLowerCase().trim() === currentVerb.past.split("/")[0].toLowerCase().trim() ? (
                        ...
                        {submitted && answers.past.toLowerCase().trim() !== currentVerb.past.split("/")[0].toLowerCase().trim() && (
                            <p className="text-xs text-green-600 font-medium mt-1 ml-1">Đáp án: {currentVerb.past}</p>
                        )}
                    </div>

                    <div className="space-y-2">
                        <label className="text-sm font-semibold ml-1">Phân từ II (V3)</label>
                        <div className="relative">
                            <Input
                                placeholder="Nhập V3..."
                                className={cn(
                                ...
                                onChange={(e) => setAnswers({ ...answers, participle: e.target.value })}
                                disabled={submitted}
                            />
                            {submitted &&
                                (answers.participle.toLowerCase().trim() ===
                        ...
                        {submitted &&
                            answers.participle.toLowerCase().trim() !== currentVerb.participle.split("/")[0].toLowerCase().trim() && (
                                <p className="text-xs text-green-600 font-medium mt-1 ml-1">Đáp án: {currentVerb.participle}</p>
                            )}
                    </div>
                </div>
                ...
                <div className="mt-10">
                    {!submitted ? (
                        <Button
                            className="w-full h-14 text-lg font-bold rounded-xl shadow-lg hover:translate-y-[-2px] transition-all"
                            onClick={checkAnswer}
                        ...
                        >
                            {currentIndex < quizVerbs.length - 1 ? "Tiếp theo" : "Xem kết quả"}
                        </Button>
                    )}
                </div>
            </div>
