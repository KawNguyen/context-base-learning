...
export function ImageDescriptionInterface({ initialIndex = 0 }: ImageDescriptionInterfaceProps) {
    ...
    const startReading = useCallback(() => {
        ...
        const readNext = () => {
            ...
            const utterance = new SpeechSynthesisUtterance(
                `Option ${String.fromCharCode(65 + index)}: ${currentQuestion.options[index]}`
            );
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-4">
                    <Button
                        ...
                        className={cn("gap-2 min-w-[120px]", isReading && "text-primary border-primary")}
                    >
                        {isReading ? <Square size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {currentQuestion.options.map((option, index) => (
                            <OptionButton
                                ...
                                isWrong={isAnswered && index === selectedOption && index !== currentQuestion.correct}
                        ...
                        <div className="animate-in slide-in-from-bottom-2 fade-in duration-300">
                            <Alert className={cn(
                                ...
                                <div className="flex items-start gap-4">
                                    <Info className={cn("size-6 shrink-0 mt-0.5", selectedOption === currentQuestion.correct ? "text-green-500" : "text-destructive")} />
                                    <div className="space-y-2">
                                        <AlertTitle className={cn(
                                            "text-xl font-extrabold",
                                            selectedOption === currentQuestion.correct ? "text-green-600" : "text-destructive"
                                        )}>
                                            {selectedOption === currentQuestion.correct ? "Excellent!" : "Oops! Not quite."}
                                        </AlertTitle>
                    ...
                    <div className="pt-4">
                        {!isAnswered ? (
                            ...
                            <Button
                                ...
                                onClick={handleNext}
                            >
                                {currentIndex < imageQuestions.length - 1 ? "Next Photo" : "View Results"}
