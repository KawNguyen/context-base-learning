...
export function DialogueInterface({ level: paramLevel }: DialogueInterfaceProps) {
    const validLevels: CEFRLevel[] = ["A1", "A2", "B1", "B2", "C1", "C2"];
    const level = validLevels.includes(paramLevel as CEFRLevel) ? paramLevel as CEFRLevel : null;
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="max-w-4xl mx-auto">
                    <div className="flex items-center gap-4 mb-8">
                        <Button variant="ghost" size="icon" onClick={() => router.push("/dialogue")}>
                                ...
                                <CardContent className="p-6 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-muted-foreground">
                                            {dialogue.lines.length} lines • {dialogue.questions.length} questions
                                        </p>
                                    </div>
                ...
                <Card className="max-w-2xl mx-auto shadow-xl border-t-4 border-t-primary">
                    ...
                    <CardContent className="text-center p-8">
                        ...
                        <p className="text-xl mb-8 font-medium">
                            You correctly answered {score} out of {dialogue.questions.length} questions.
                        </p>
                        <div className="flex gap-4 justify-center">
                            <Button onClick={() => setSelectedDialogue(null)} size="lg" className="px-8">
                                Try Another
                            </Button>
                            <Button onClick={() => router.push("/dialogue")} variant="outline" size="lg">
                                Change Level
                            </Button>
                        </div>
                    </CardContent>
            ...
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <Button variant="ghost" size="icon" onClick={() => setSelectedDialogue(null)}>
                            <ArrowLeft className="w-6 h-6" />
                        </Button>
                ...
                <div className="bg-muted/30 rounded-2xl p-6 mb-8 space-y-6 shadow-inner border">
                    {dialogue.lines.map((line, idx) => (
                        <div
                            key={idx}
                            className="flex flex-col max-w-[90%] items-start"
                        >
                            <span className="text-xs font-bold text-muted-foreground mb-1 px-2 uppercase tracking-wider">
                                {line.speaker}
                            </span>
                            <div
                                className="p-4 rounded-xl text-base leading-relaxed shadow-sm bg-card text-card-foreground border"
                            >
                                {line.text}
                            </div>
                        </div>
                ...
                <Card className="border-2 border-primary/20 overflow-hidden">
                    <div className="h-2 bg-muted">
                        <div
                            ...
                            style={{ width: `${((currentQuestion + 1) / dialogue.questions.length) * 100}%` }}
                        />
                    </div>
                    ...
                    <CardContent className="p-8">
                        ...
                        <div className="grid gap-3">
                            {dialogue.questions[currentQuestion].options.map((option, index) => {
                                const isSelected = selectedAnswer === index;
                                const isCorrect = index === dialogue.questions[currentQuestion].correct;
                            ...
                            <div className={cn(
                                ...
                                <div>
                                    <p className={cn(
                                        ...
                                        selectedAnswer === dialogue.questions[currentQuestion].correct ? "text-green-700" : "text-red-700"
                                    )}>
                                        {selectedAnswer === dialogue.questions[currentQuestion].correct ? "Well done!" : "Not quite right..."}
                                    </p>
                                    <p className="text-muted-foreground">
                        ...
                        {showExplanation && (
                            <div className="mt-8 flex justify-end">
                                <Button onClick={handleNextQuestion} size="lg" className="px-10">
                                    {currentQuestion < dialogue.questions.length - 1 ? "Next Question" : "View Results"}
                                </Button>
                            </div>
                        )}
                    </CardContent>
