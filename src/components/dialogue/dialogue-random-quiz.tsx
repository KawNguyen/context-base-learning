...
export function DialogueRandomQuiz({ level }: DialogueRandomQuizProps) {
  const router = useRouter();
  const params = useParams();
  const levelSlug = params.level as string;

  const [randomDialogues, setRandomDialogues] = useState<Dialogue[]>([]);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          {/* All Dialogues - No grid, stacked vertically like real  */}
          <div className="space-y-6">
