import { LevelListeningData } from "../types";

export const A1_LISTENING: LevelListeningData = {
  dictations: [
    {
      id: "a1-dict-1",
      title: "Simple Greeting",
      level: "A1",
      content: "Hello, my name is Tom and I am ten years old.",
      answer: "Hello, my name is Tom and I am ten years old.",
    },
    {
      id: "a1-dict-2",
      title: "Family Member",
      level: "A1",
      content: "I have one sister, her name is Lucy.",
      answer: "I have one sister, her name is Lucy.",
    },
    {
      id: "a1-dict-3",
      title: "School Subject",
      level: "A1",
      content: "My favorite subject is Art because I love to draw pictures.",
      answer: "My favorite subject is Art because I love to draw pictures.",
    },
    {
      id: "a1-dict-4",
      title: "Morning Habit",
      level: "A1",
      content: "I brush my teeth and wash my face every morning.",
      answer: "I brush my teeth and wash my face every morning.",
    },
    {
      id: "a1-dict-5",
      title: "Weather Today",
      level: "A1",
      content: "Today is a beautiful day with a bright blue sky.",
      answer: "Today is a beautiful day with a bright blue sky.",
    },
    {
      id: "a1-dict-6",
      title: "Lunch Meal",
      level: "A1",
      content: "I usually have a cheese sandwich and an apple for lunch.",
      answer: "I usually have a cheese sandwich and an apple for lunch.",
    },
    {
      id: "a1-dict-7",
      title: "Reading Time",
      level: "A1",
      content:
        "My sister loves to read funny stories before she goes to sleep.",
      answer: "My sister loves to read funny stories before she goes to sleep.",
    },
    {
      id: "a1-dict-8",
      title: "Weekend Trip",
      level: "A1",
      content: "On Saturdays, we often go to the big park near our house.",
      answer: "On Saturdays, we often go to the big park near our house.",
    },
    {
      id: "a1-dict-9",
      title: "Pet Dog",
      level: "A1",
      content: "We have a very cute pet dog named Max. He is black and white.",
      answer: "We have a very cute pet dog named Max. He is black and white.",
    },
    {
      id: "a1-dict-10",
      title: "Living Place",
      level: "A1",
      content: "London is a very big and busy city in England.",
      answer: "London is a very big and busy city in England.",
    },
    {
      id: "a1-dict-11",
      title: "Favorite Color",
      level: "A1",
      content: "My favorite color is red because it is bright and happy.",
      answer: "My favorite color is red because it is bright and happy.",
    },
    {
      id: "a1-dict-12",
      title: "Grandparents",
      level: "A1",
      content: "Sometimes we visit my grandparents in the countryside.",
      answer: "Sometimes we visit my grandparents in the countryside.",
    },
    {
      id: "a1-dict-13",
      title: "Best Friend",
      level: "A1",
      content:
        "My best friend is Sarah, she is a very kind and cheerful person.",
      answer:
        "My best friend is Sarah, she is a very kind and cheerful person.",
    },
    {
      id: "a1-dict-14",
      title: "Sports Activity",
      level: "A1",
      content: "I like to play football with my friends after school.",
      answer: "I like to play football with my friends after school.",
    },
    {
      id: "a1-dict-15",
      title: "School Start",
      level: "A1",
      content: "My school starts at eight am and ends at four pm.",
      answer: "My school starts at eight am and ends at four pm.",
    },
    {
      id: "a1-dict-16",
      title: "Drawing Hobby",
      level: "A1",
      content: "I have a big sketchbook where I draw animals and cars.",
      answer: "I have a big sketchbook where I draw animals and cars.",
    },
    {
      id: "a1-dict-17",
      title: "Breakfast Drinks",
      level: "A1",
      content:
        "I usually have a tall glass of fresh orange juice for breakfast.",
      answer:
        "I usually have a tall glass of fresh orange juice for breakfast.",
    },
    {
      id: "a1-dict-18",
      title: "Library Day",
      level: "A1",
      content: "She goes to the local library every Friday to borrow books.",
      answer: "She goes to the local library every Friday to borrow books.",
    },
    {
      id: "a1-dict-19",
      title: "Garden Flowers",
      level: "A1",
      content: "There are many red roses and yellow daisies in the garden.",
      answer: "There are many red roses and yellow daisies in the garden.",
    },
    {
      id: "a1-dict-20",
      title: "Future Job",
      level: "A1",
      content: "I want to be a doctor when I grow up to help sick people.",
      answer: "I want to be a doctor when I grow up to help sick people.",
    },
  ],
  comprehensions: [
    {
      id: "a1-comp-1",
      title: "Tom's Daily Life",
      level: "A1",
      content:
        "Hello, I am Tom. I am ten years old. I live in London. My house is small but cozy. I have one sister, Lucy. She is eight. We go to school at 8 am. I like Art, and she likes Reading. We have a pet dog named Max. Max is black and white.",
      transcript:
        "Hello, I am Tom. I am ten years old. I live in London. My house is small but cozy. I have one sister, Lucy. She is eight. We go to school at 8 am. I like Art, and she likes Reading. We have a pet dog named Max. Max is black and white.",
      questions: [
        {
          question: "How old is Tom?",
          options: [
            {
              isCorrect: false,
              option: "Eight",
            },
            {
              isCorrect: false,
              option: "Nine",
            },
            {
              isCorrect: true,
              option: "Ten",
            },
            {
              isCorrect: false,
              option: "Eleven",
            },
          ],
          explanation: "In the passage: 'I am ten years old.'",
        },
        {
          question: "Where does Tom live?",
          options: [
            {
              isCorrect: false,
              option: "Paris",
            },
            {
              isCorrect: true,
              option: "London",
            },
            {
              isCorrect: false,
              option: "New York",
            },
            {
              isCorrect: false,
              option: "Tokyo",
            },
          ],
          explanation: "In the passage: 'I live in London.'",
        },
        {
          question: "How old is Lucy?",
          options: [
            {
              isCorrect: false,
              option: "Seven",
            },
            {
              isCorrect: true,
              option: "Eight",
            },
            {
              isCorrect: false,
              option: "Nine",
            },
            {
              isCorrect: false,
              option: "Ten",
            },
          ],
          explanation: "In the passage: 'She is eight.'",
        },
        {
          question: "What time do they go to school?",
          options: [
            {
              isCorrect: false,
              option: "7 am",
            },
            {
              isCorrect: true,
              option: "8 am",
            },
            {
              isCorrect: false,
              option: "9 am",
            },
            {
              isCorrect: false,
              option: "10 am",
            },
          ],
          explanation: "In the passage: 'We go to school at 8 am.'",
        },
        {
          question: "What is Tom's favorite subject?",
          options: [
            {
              isCorrect: false,
              option: "Maths",
            },
            {
              isCorrect: false,
              option: "Science",
            },
            {
              isCorrect: true,
              option: "Art",
            },
            {
              isCorrect: false,
              option: "Reading",
            },
          ],
          explanation: "In the passage: 'I like Art.'",
        },
        {
          question: "What color is Max the dog?",
          options: [
            {
              isCorrect: false,
              option: "Brown",
            },
            {
              isCorrect: false,
              option: "Black",
            },
            {
              isCorrect: true,
              option: "Black and white",
            },
            {
              isCorrect: false,
              option: "White",
            },
          ],
          explanation: "In the passage: 'Max is black and white.'",
        },
      ],
    },
    {
      id: "a1-comp-2",
      title: "My Family Weekend",
      level: "A1",
      content:
        "Every Saturday, my family goes to the park. My father is a doctor. My mother is a teacher. We play football together. On Sunday, we visit our grandparents. They live in the countryside. My grandma makes sweet cakes. I love my family very much.",
      transcript:
        "Every Saturday, my family goes to the park. My father is a doctor. My mother is a teacher. We play football together. On Sunday, we visit our grandparents. They live in the countryside. My grandma makes sweet cakes. I love my family very much.",
      questions: [
        {
          question: "When do they go to the park?",
          options: [
            {
              isCorrect: false,
              option: "Monday",
            },
            {
              isCorrect: false,
              option: "Friday",
            },
            {
              isCorrect: true,
              option: "Saturday",
            },
            {
              isCorrect: false,
              option: "Sunday",
            },
          ],
          explanation:
            "In the passage: 'Every Saturday, my family goes to the park.'",
        },
        {
          question: "What is the father's job?",
          options: [
            {
              isCorrect: false,
              option: "Teacher",
            },
            {
              isCorrect: true,
              option: "Doctor",
            },
            {
              isCorrect: false,
              option: "Driver",
            },
            {
              isCorrect: false,
              option: "Farmer",
            },
          ],
          explanation: "In the passage: 'My father is a doctor.'",
        },
        {
          question: "What do they play together?",
          options: [
            {
              isCorrect: false,
              option: "Tennis",
            },
            {
              isCorrect: true,
              option: "Football",
            },
            {
              isCorrect: false,
              option: "Basketball",
            },
            {
              isCorrect: false,
              option: "Swimming",
            },
          ],
          explanation: "In the passage: 'We play football together.'",
        },
        {
          question: "Who do they visit on Sunday?",
          options: [
            {
              isCorrect: false,
              option: "Friends",
            },
            {
              isCorrect: false,
              option: "Aunty",
            },
            {
              isCorrect: true,
              option: "Grandparents",
            },
            {
              isCorrect: false,
              option: "Neighbors",
            },
          ],
          explanation:
            "In the passage: 'On Sunday, we visit our grandparents.'",
        },
        {
          question: "Where do the grandparents live?",
          options: [
            {
              isCorrect: false,
              option: "In the city",
            },
            {
              isCorrect: false,
              option: "In London",
            },
            {
              isCorrect: true,
              option: "In the countryside",
            },
            {
              isCorrect: false,
              option: "At the park",
            },
          ],
          explanation: "In the passage: 'They live in the countryside.'",
        },
        {
          question: "What does grandma make?",
          options: [
            {
              isCorrect: false,
              option: "Pizza",
            },
            {
              isCorrect: true,
              option: "Cakes",
            },
            {
              isCorrect: false,
              option: "Soup",
            },
            {
              isCorrect: false,
              option: "Bread",
            },
          ],
          explanation: "In the passage: 'My grandma makes sweet cakes.'",
        },
      ],
    },
    {
      id: "a1-comp-3",
      title: "Sarah, My Best Friend",
      level: "A1",
      content:
        "Sarah is my best friend. She is tall and thin. She has blonde hair. She is very funny. We play tennis every Saturday morning. We also like chocolate ice cream. Sarah likes reading adventure stories. I am happy to have her as a friend.",
      transcript:
        "Sarah is my best friend. She is tall and thin. She has blonde hair. She is very funny. We play tennis every Saturday morning. We also like chocolate ice cream. Sarah likes reading adventure stories. I am happy to have her as a friend.",
      questions: [
        {
          question: "Who is Sarah?",
          options: [
            {
              isCorrect: false,
              option: "A sister",
            },
            {
              isCorrect: false,
              option: "A mother",
            },
            {
              isCorrect: false,
              option: "A teacher",
            },
            {
              isCorrect: true,
              option: "A best friend",
            },
          ],
          explanation: "In the passage: 'Sarah is my best friend.'",
        },
        {
          question: "What does Sarah look like?",
          options: [
            {
              isCorrect: false,
              option: "Short and fat",
            },
            {
              isCorrect: true,
              option: "Tall and thin",
            },
            {
              isCorrect: false,
              option: "Short and thin",
            },
            {
              isCorrect: false,
              option: "Tall and fat",
            },
          ],
          explanation: "In the passage: 'She is tall and thin.'",
        },
        {
          question: "What color is her hair?",
          options: [
            {
              isCorrect: false,
              option: "Black",
            },
            {
              isCorrect: false,
              option: "Brown",
            },
            {
              isCorrect: true,
              option: "Blonde",
            },
            {
              isCorrect: false,
              option: "Red",
            },
          ],
          explanation: "In the passage: 'She has blonde hair.'",
        },
        {
          question: "What sport do they play?",
          options: [
            {
              isCorrect: false,
              option: "Football",
            },
            {
              isCorrect: true,
              option: "Tennis",
            },
            {
              isCorrect: false,
              option: "Basketball",
            },
            {
              isCorrect: false,
              option: "Golf",
            },
          ],
          explanation:
            "In the passage: 'We play tennis every Saturday morning.'",
        },
        {
          question: "What flavor of ice cream do they like?",
          options: [
            {
              isCorrect: false,
              option: "Vanilla",
            },
            {
              isCorrect: false,
              option: "Strawberry",
            },
            {
              isCorrect: true,
              option: "Chocolate",
            },
            {
              isCorrect: false,
              option: "Mango",
            },
          ],
          explanation: "In the passage: 'We also like chocolate ice cream.'",
        },
        {
          question: "What kind of stories does Sarah like?",
          options: [
            {
              isCorrect: false,
              option: "Horror",
            },
            {
              isCorrect: false,
              option: "History",
            },
            {
              isCorrect: true,
              option: "Adventure",
            },
            {
              isCorrect: false,
              option: "Science",
            },
          ],
          explanation:
            "In the passage: 'Sarah likes reading adventure stories.'",
        },
      ],
    },
    {
      id: "a1-comp-4",
      title: "My Cozy House",
      level: "A1",
      content:
        "My house is white with a red roof. It has two bedrooms and one kitchen. My bedroom has a big window. I can see the garden from my window. The garden has red roses and yellow flowers. My mother loves to cook in the kitchen. I feel very safe in my house.",
      transcript:
        "My house is white with a red roof. It has two bedrooms and one kitchen. My bedroom has a big window. I can see the garden from my window. The garden has red roses and yellow flowers. My mother loves to cook in the kitchen. I feel very safe in my house.",
      questions: [
        {
          question: "What color is the house?",
          options: [
            {
              isCorrect: false,
              option: "Yellow",
            },
            {
              isCorrect: false,
              option: "Red",
            },
            {
              isCorrect: true,
              option: "White",
            },
            {
              isCorrect: false,
              option: "Blue",
            },
          ],
          explanation: "In the passage: 'My house is white...'",
        },
        {
          question: "What color is the roof?",
          options: [
            {
              isCorrect: true,
              option: "Red",
            },
            {
              isCorrect: false,
              option: "Black",
            },
            {
              isCorrect: false,
              option: "Green",
            },
            {
              isCorrect: false,
              option: "White",
            },
          ],
          explanation: "In the passage: '...with a red roof.'",
        },
        {
          question: "How many bedrooms are there?",
          options: [
            {
              isCorrect: false,
              option: "One",
            },
            {
              isCorrect: true,
              option: "Two",
            },
            {
              isCorrect: false,
              option: "Three",
            },
            {
              isCorrect: false,
              option: "Four",
            },
          ],
          explanation: "In the passage: 'It has two bedrooms...'",
        },
        {
          question: "What can the person see from the window?",
          options: [
            {
              isCorrect: false,
              option: "A car",
            },
            {
              isCorrect: false,
              option: "The street",
            },
            {
              isCorrect: true,
              option: "The garden",
            },
            {
              isCorrect: false,
              option: "A school",
            },
          ],
          explanation: "In the passage: 'I can see the garden from my window.'",
        },
        {
          question: "What flowers are in the garden?",
          options: [
            {
              isCorrect: false,
              option: "Lilies",
            },
            {
              isCorrect: true,
              option: "Red roses",
            },
            {
              isCorrect: false,
              option: "Sunflowers",
            },
            {
              isCorrect: false,
              option: "Tulips",
            },
          ],
          explanation: "In the passage: 'The garden has red roses...' ",
        },
        {
          question: "Who loves to cook in the kitchen?",
          options: [
            {
              isCorrect: false,
              option: "The father",
            },
            {
              isCorrect: false,
              option: "The writer",
            },
            {
              isCorrect: true,
              option: "The mother",
            },
            {
              isCorrect: false,
              option: "The sister",
            },
          ],
          explanation:
            "In the passage: 'My mother loves to cook in the kitchen.'",
        },
      ],
    },
    {
      id: "a1-comp-5",
      title: "Morning Routine",
      level: "A1",
      content:
        "The alarm rings at six-thirty. I get up and wash my face. I brush my teeth with minty toothpaste. Then I put on my blue shirt. At seven o'clock, I eat breakfast. I have toast and a glass of milk. At seven-thirty, I go to the bus stop.",
      transcript:
        "The alarm rings at six-thirty. I get up and wash my face. I brush my teeth with minty toothpaste. Then I put on my blue shirt. At seven o'clock, I eat breakfast. I have toast and a glass of milk. At seven-thirty, I go to the bus stop.",
      questions: [
        {
          question: "What time does the alarm ring?",
          options: [
            {
              isCorrect: false,
              option: "6:00",
            },
            {
              isCorrect: true,
              option: "6:30",
            },
            {
              isCorrect: false,
              option: "7:00",
            },
            {
              isCorrect: false,
              option: "7:30",
            },
          ],
          explanation: "In the passage: 'The alarm rings at six-thirty.'",
        },
        {
          question: "What does the person do first?",
          options: [
            {
              isCorrect: false,
              option: "Eat breakfast",
            },
            {
              isCorrect: false,
              option: "Put on a shirt",
            },
            {
              isCorrect: true,
              option: "Wash face",
            },
            {
              isCorrect: false,
              option: "Go to bus stop",
            },
          ],
          explanation: "In the passage: 'I get up and wash my face.'",
        },
        {
          question: "What color is the shirt?",
          options: [
            {
              isCorrect: false,
              option: "Red",
            },
            {
              isCorrect: false,
              option: "White",
            },
            {
              isCorrect: true,
              option: "Blue",
            },
            {
              isCorrect: false,
              option: "Black",
            },
          ],
          explanation: "In the passage: 'Then I put on my blue shirt.'",
        },
        {
          question: "What time is breakfast?",
          options: [
            {
              isCorrect: false,
              option: "6:30",
            },
            {
              isCorrect: true,
              option: "7:00",
            },
            {
              isCorrect: false,
              option: "7:30",
            },
            {
              isCorrect: false,
              option: "8:00",
            },
          ],
          explanation: "In the passage: 'At seven o'clock, I eat breakfast.'",
        },
        {
          question: "What does the person have for breakfast?",
          options: [
            {
              isCorrect: false,
              option: "Eggs",
            },
            {
              isCorrect: false,
              option: "Cereal",
            },
            {
              isCorrect: false,
              option: "Soup",
            },
            {
              isCorrect: true,
              option: "Toast and milk",
            },
          ],
          explanation: "In the passage: 'I have toast and a glass of milk.'",
        },
        {
          question: "When does the person go to the bus stop?",
          options: [
            {
              isCorrect: false,
              option: "7:00",
            },
            {
              isCorrect: true,
              option: "7:30",
            },
            {
              isCorrect: false,
              option: "8:00",
            },
            {
              isCorrect: false,
              option: "8:30",
            },
          ],
          explanation:
            "In the passage: 'At seven-thirty, I go to the bus stop.'",
        },
      ],
    },
    {
      id: "a1-comp-6",
      title: "Luna, The Cat",
      level: "A1",
      content:
        "Luna is a white cat. She is very fluffy. She has big green eyes. Luna is very quiet. She likes to sleep on the sofa. Sometimes she plays with a toy mouse. Luna drinks fresh milk every morning. She does not like baths because she hates water.",
      transcript:
        "Luna is a white cat. She is very fluffy. She has big green eyes. Luna is very quiet. She likes to sleep on the sofa. Sometimes she plays with a toy mouse. Luna drinks fresh milk every morning. She does not like baths because she hates water.",
      questions: [
        {
          question: "What kind of animal is Luna?",
          options: [
            {
              isCorrect: false,
              option: "A dog",
            },
            {
              isCorrect: true,
              option: "A cat",
            },
            {
              isCorrect: false,
              option: "A rabbit",
            },
            {
              isCorrect: false,
              option: "A bird",
            },
          ],
          explanation: "In the passage: 'Luna is a white cat.'",
        },
        {
          question: "What color is Luna?",
          options: [
            {
              isCorrect: false,
              option: "Black",
            },
            {
              isCorrect: false,
              option: "Brown",
            },
            {
              isCorrect: true,
              option: "White",
            },
            {
              isCorrect: false,
              option: "Grey",
            },
          ],
          explanation: "In the passage: 'Luna is a white cat.'",
        },
        {
          question: "What color are her eyes?",
          options: [
            {
              isCorrect: false,
              option: "Blue",
            },
            {
              isCorrect: true,
              option: "Green",
            },
            {
              isCorrect: false,
              option: "Yellow",
            },
            {
              isCorrect: false,
              option: "Brown",
            },
          ],
          explanation: "In the passage: 'She has big green eyes.'",
        },
        {
          question: "Where does she like to sleep?",
          options: [
            {
              isCorrect: false,
              option: "In a bed",
            },
            {
              isCorrect: false,
              option: "On the floor",
            },
            {
              isCorrect: true,
              option: "On the sofa",
            },
            {
              isCorrect: false,
              option: "In the garden",
            },
          ],
          explanation: "In the passage: 'She likes to sleep on the sofa.'",
        },
        {
          question: "What does she play with?",
          options: [
            {
              isCorrect: false,
              option: "A ball",
            },
            {
              isCorrect: true,
              option: "A toy mouse",
            },
            {
              isCorrect: false,
              option: "A string",
            },
            {
              isCorrect: false,
              option: "A doll",
            },
          ],
          explanation:
            "In the passage: 'Sometimes she plays with a toy mouse.'",
        },
        {
          question: "What does Luna hate?",
          options: [
            {
              isCorrect: false,
              option: "Milk",
            },
            {
              isCorrect: false,
              option: "Mice",
            },
            {
              isCorrect: false,
              option: "Sofas",
            },
            {
              isCorrect: true,
              option: "Water",
            },
          ],
          explanation: "In the passage: '...she hates water.'",
        },
      ],
    },
    {
      id: "a1-comp-7",
      title: "A Hobby: Drawing",
      level: "A1",
      content:
        "I have a hobby. It is drawing. I have a big sketchbook. I use pencils and crayons to draw. I like to draw animals. My favorite animal to draw is a lion. I also draw fast cars. I draw in my bedroom because it is very quiet there.",
      transcript:
        "I have a hobby. It is drawing. I have a big sketchbook. I use pencils and crayons to draw. I like to draw animals. My favorite animal to draw is a lion. I also draw fast cars. I draw in my bedroom because it is very quiet there.",
      questions: [
        {
          question: "What is the hobby?",
          options: [
            {
              isCorrect: false,
              option: "Reading",
            },
            {
              isCorrect: true,
              option: "Drawing",
            },
            {
              isCorrect: false,
              option: "Singing",
            },
            {
              isCorrect: false,
              option: "Cooking",
            },
          ],
          explanation: "In the passage: 'It is drawing.'",
        },
        {
          question: "What does the person draw in?",
          options: [
            {
              isCorrect: false,
              option: "A book",
            },
            {
              isCorrect: false,
              option: "A notebook",
            },
            {
              isCorrect: true,
              option: "A sketchbook",
            },
            {
              isCorrect: false,
              option: "On paper",
            },
          ],
          explanation: "In the passage: 'I have a big sketchbook.'",
        },
        {
          question: "What tools does the person use?",
          options: [
            {
              isCorrect: false,
              option: "Paint",
            },
            {
              isCorrect: false,
              option: "Markers",
            },
            {
              isCorrect: true,
              option: "Pencils and crayons",
            },
            {
              isCorrect: false,
              option: "Pens",
            },
          ],
          explanation: "In the passage: 'I use pencils and crayons...'",
        },
        {
          question: "What does the person like to draw?",
          options: [
            {
              isCorrect: false,
              option: "Houses",
            },
            {
              isCorrect: false,
              option: "Flowers",
            },
            {
              isCorrect: true,
              option: "Animals",
            },
            {
              isCorrect: false,
              option: "People",
            },
          ],
          explanation: "In the passage: 'I like to draw animals.'",
        },
        {
          question: "What is the favorite animal to draw?",
          options: [
            {
              isCorrect: false,
              option: "Tiger",
            },
            {
              isCorrect: false,
              option: "Cat",
            },
            {
              isCorrect: true,
              option: "Lion",
            },
            {
              isCorrect: false,
              option: "Elephant",
            },
          ],
          explanation:
            "In the passage: 'My favorite animal to draw is a lion.'",
        },
        {
          question: "Where does the person draw?",
          options: [
            {
              isCorrect: false,
              option: "In the living room",
            },
            {
              isCorrect: false,
              option: "At school",
            },
            {
              isCorrect: false,
              option: "In the garden",
            },
            {
              isCorrect: true,
              option: "In the bedroom",
            },
          ],
          explanation: "In the passage: 'I draw in my bedroom...'",
        },
      ],
    },
    {
      id: "a1-comp-8",
      title: "The Local Library",
      level: "A1",
      content:
        "I go to the library every Friday afternoon. The library is very big. There are many books on the shelves. I borrow three books every week. I like reading stories about magic. My friend Jack goes with me. We read quietly for one hour.",
      transcript:
        "I go to the library every Friday afternoon. The library is very big. There are many books on the shelves. I borrow three books every week. I like reading stories about magic. My friend Jack goes with me. We read quietly for one hour.",
      questions: [
        {
          question: "When does the person go to the library?",
          options: [
            {
              isCorrect: false,
              option: "Monday",
            },
            {
              isCorrect: true,
              option: "Friday",
            },
            {
              isCorrect: false,
              option: "Saturday",
            },
            {
              isCorrect: false,
              option: "Sunday",
            },
          ],
          explanation:
            "In the passage: 'I go to the library every Friday afternoon.'",
        },
        {
          question: "Is the library small?",
          options: [
            {
              isCorrect: false,
              option: "Yes, it is",
            },
            {
              isCorrect: true,
              option: "No, it is very big",
            },
            {
              isCorrect: false,
              option: "It is old",
            },
            {
              isCorrect: false,
              option: "It is new",
            },
          ],
          explanation: "In the passage: 'The library is very big.'",
        },
        {
          question: "How many books does the person borrow?",
          options: [
            {
              isCorrect: false,
              option: "One",
            },
            {
              isCorrect: false,
              option: "Two",
            },
            {
              isCorrect: true,
              option: "Three",
            },
            {
              isCorrect: false,
              option: "Four",
            },
          ],
          explanation: "In the passage: 'I borrow three books every week.'",
        },
        {
          question: "What kind of stories does the person like?",
          options: [
            {
              isCorrect: false,
              option: "History",
            },
            {
              isCorrect: true,
              option: "Magic",
            },
            {
              isCorrect: false,
              option: "Science",
            },
            {
              isCorrect: false,
              option: "War",
            },
          ],
          explanation: "In the passage: 'I like reading stories about magic.'",
        },
        {
          question: "Who goes with the person?",
          options: [
            {
              isCorrect: false,
              option: "His mother",
            },
            {
              isCorrect: false,
              option: "His sister",
            },
            {
              isCorrect: true,
              option: "His friend Jack",
            },
            {
              isCorrect: false,
              option: "His teacher",
            },
          ],
          explanation: "In the passage: 'My friend Jack goes with me.'",
        },
        {
          question: "How long do they stay there?",
          options: [
            {
              isCorrect: false,
              option: "30 minutes",
            },
            {
              isCorrect: true,
              option: "One hour",
            },
            {
              isCorrect: false,
              option: "Two hours",
            },
            {
              isCorrect: false,
              option: "All day",
            },
          ],
          explanation: "In the passage: 'We read quietly for one hour.'",
        },
      ],
    },
    {
      id: "a1-comp-9",
      title: "Playtime at the Park",
      level: "A1",
      content:
        "The park is my favorite place. I go there with my friends. We play on the slides and swings. I like the slides because they are fast. Some children play with a ball on the grass. People often walk their dogs there. It is a very happy and fun place.",
      transcript:
        "The park is my favorite place. I go there with my friends. We play on the slides and swings. I like the slides because they are fast. Some children play with a ball on the grass. People often walk their dogs there. It is a very happy and fun place.",
      questions: [
        {
          question: "What is the person's favorite place?",
          options: [
            {
              isCorrect: false,
              option: "School",
            },
            {
              isCorrect: false,
              option: "Home",
            },
            {
              isCorrect: false,
              option: "Library",
            },
            {
              isCorrect: true,
              option: "Park",
            },
          ],
          explanation: "In the passage: 'The park is my favorite place.'",
        },
        {
          question: "Who does the person go with?",
          options: [
            {
              isCorrect: false,
              option: "Family",
            },
            {
              isCorrect: true,
              option: "Friends",
            },
            {
              isCorrect: false,
              option: "Teacher",
            },
            {
              isCorrect: false,
              option: "Alone",
            },
          ],
          explanation: "In the passage: 'I go there with my friends.'",
        },
        {
          question: "What do they play on?",
          options: [
            {
              isCorrect: false,
              option: "Bikes",
            },
            {
              isCorrect: false,
              option: "Computers",
            },
            {
              isCorrect: true,
              option: "Slides and swings",
            },
            {
              isCorrect: false,
              option: "Musical instruments",
            },
          ],
          explanation: "In the passage: 'We play on the slides and swings.'",
        },
        {
          question: "Why does the person like the slides?",
          options: [
            {
              isCorrect: false,
              option: "They are big",
            },
            {
              isCorrect: true,
              option: "They are fast",
            },
            {
              isCorrect: false,
              option: "They are red",
            },
            {
              isCorrect: false,
              option: "They are new",
            },
          ],
          explanation:
            "In the passage: 'I like the slides because they are fast.'",
        },
        {
          question: "Where do children play with a ball?",
          options: [
            {
              isCorrect: false,
              option: "In the street",
            },
            {
              isCorrect: true,
              option: "On the grass",
            },
            {
              isCorrect: false,
              option: "At school",
            },
            {
              isCorrect: false,
              option: "At home",
            },
          ],
          explanation:
            "In the passage: 'Some children play with a ball on the grass.'",
        },
        {
          question: "What do people do with their dogs?",
          options: [
            {
              isCorrect: false,
              option: "Feed them",
            },
            {
              isCorrect: false,
              option: "Wash them",
            },
            {
              isCorrect: true,
              option: "Walk them",
            },
            {
              isCorrect: false,
              option: "Buy them",
            },
          ],
          explanation: "In the passage: 'People often walk their dogs there.'",
        },
      ],
    },
    {
      id: "a1-comp-10",
      title: "Future Dreams",
      level: "A1",
      content:
        "I want to be a doctor when I grow up. My father is a doctor too. He works at a big hospital. He helps many sick people every day. I want to learn many things at school. I want to make people feel better and happy. Helping others is important.",
      transcript:
        "I want to be a doctor when I grow up. My father is a doctor too. He works at a big hospital. He helps many sick people every day. I want to learn many things at school. I want to make people feel better and happy. Helping others is important.",
      questions: [
        {
          question: "What does the person want to be?",
          options: [
            {
              isCorrect: false,
              option: "Teacher",
            },
            {
              isCorrect: true,
              option: "Doctor",
            },
            {
              isCorrect: false,
              option: "Driver",
            },
            {
              isCorrect: false,
              option: "Farmer",
            },
          ],
          explanation: "In the passage: 'I want to be a doctor...'",
        },
        {
          question: "Who else is a doctor?",
          options: [
            {
              isCorrect: false,
              option: "The mother",
            },
            {
              isCorrect: true,
              option: "The father",
            },
            {
              isCorrect: false,
              option: "The grandfather",
            },
            {
              isCorrect: false,
              option: "The friend",
            },
          ],
          explanation: "In the passage: 'My father is a doctor too.'",
        },
        {
          question: "Where does the father work?",
          options: [
            {
              isCorrect: false,
              option: "At a school",
            },
            {
              isCorrect: true,
              option: "At a big hospital",
            },
            {
              isCorrect: false,
              option: "In an office",
            },
            {
              isCorrect: false,
              option: "At home",
            },
          ],
          explanation: "In the passage: 'He works at a big hospital.'",
        },
        {
          question: "Who does the father help?",
          options: [
            {
              isCorrect: false,
              option: "Animals",
            },
            {
              isCorrect: false,
              option: "Students",
            },
            {
              isCorrect: true,
              option: "Sick people",
            },
            {
              isCorrect: false,
              option: "Teachers",
            },
          ],
          explanation: "In the passage: 'He helps many sick people every day.'",
        },
        {
          question: "What does the person want to do for people?",
          options: [
            {
              isCorrect: false,
              option: "Make them rich",
            },
            {
              isCorrect: true,
              option: "Make them feel better",
            },
            {
              isCorrect: false,
              option: "Give them books",
            },
            {
              isCorrect: false,
              option: "Take them to the park",
            },
          ],
          explanation:
            "In the passage: 'I want to make people feel better and happy.'",
        },
        {
          question: "What is important to the person?",
          options: [
            {
              isCorrect: false,
              option: "Playing games",
            },
            {
              isCorrect: false,
              option: "Sleeping",
            },
            {
              isCorrect: true,
              option: "Helping others",
            },
            {
              isCorrect: false,
              option: "Watching TV",
            },
          ],
          explanation: "In the passage: 'Helping others is important.'",
        },
      ],
    },
  ],
};
